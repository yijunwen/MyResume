...
import com.yhaj.xr.domain.UploadParams;
...
public class Uploads {
    ...
    public static boolean deleteOldImage(HttpServletRequest request, String oldImage) throws Exception {
        // 删除旧的文件
        if (oldImage != null) {
            // 如果oldImage是空串，那么就会把整个web项目的文件夹给删掉
            FileUtils.deleteQuietly(new File(request.getServletContext().getRealPath(oldImage)));
            return true;
        }
        return false;
    }
